...
const Button: VoidComponent<ButtonProps> = ({ onClick = () => undefined, class: cls = "", text = "" }) => {
    return (
        <button
            class={`${cls} group relative cursor-pointer appearance-none rounded-lg border border-accent/30 bg-transparent px-3 py-5 text-base/tight font-medium uppercase tracking-wide text-accent/50 outline-none transition-all duration-200 ease-in-out before:absolute before:left-1/10 before:top-0 before:h-px before:w-16 before:-translate-y-px before:bg-accent before:transition-all before:duration-200 before:ease-in-out after:absolute after:bottom-0 after:right-1/10 after:h-px after:w-16 after:translate-y-px after:bg-accent after:transition-all after:duration-200 after:ease-in-out hover:border-accent-alt hover:text-accent hover:shadow-blue hover:text-shadow hover:before:left-0 hover:before:w-5 hover:after:right-0 hover:after:w-5`}
            on:click={onClick}
        >
